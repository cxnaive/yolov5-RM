...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            ps = pi[b, a, gj, gi]  # prediclosstion subset corresponding to targets
            ...
            if model.nc > 1:  # cls loss (only if multiple classes)
                t = torch.full_like(ps[:, 15:15 + model.nc], cn, device=device)  # targets
                t[range(n), tcls[i]] = cp
...
def build_targets(p, targets, model):
    ...
    # with open('targets_final.txt','w') as f:
    #         for line in targets:
    #             f.write(str(line) +'\n')
    ...
    for i in range(det.nl):
        ...
        #     for line in lks:
        #         f.write(str(line) +'\n')
        # print('gij!!!!:',torch.sum(torch.isnan(gij)))
        # exit()
